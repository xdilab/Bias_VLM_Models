...
class VLM_Physics_Textual_BBox_Dataset(Dataset):
    def __init__(self, metadata_df: pd.DataFrame, 
                 external_seg_model: nn.Module, 
                 external_seg_transform: A.Compose, 
                 device: str,
                 is_train: bool = True):
        ...
        for _, row in tqdm(metadata_df.iterrows(), total=len(metadata_df), desc="Generating Textual BBoxes"):
            ...
            try:
                ...
                bbox, has_pred_tumor = delineate_and_bbox_from_pred(
                    raw_img, external_seg_model, external_seg_transform, device
                )
            except Exception as e:
                bbox, has_pred_tumor = None, False

            
            has_tumor_gt = row['has_tumor']
            ...
            if has_tumor_gt:
                ...
                if bbox is not None:
                    x, y, w, h = bbox
                    q = (
                        f"Analyze this MRI slice. A suspicious region was detected at bounding box "
                        f"(x:{x}, y:{y}, w:{w}, h:{h}) in 256x256 space. "
                ...
                if bbox is not None and has_pred_tumor:
                   
                    x, y, w, h = bbox
                    q = (
                        f"Analyze this MRI slice. A suspicious region was detected at bounding box "
                        f"(x:{x}, y:{y}, w:{w}, h:{h}) in 256x256 space. "
                        f"Is a tumor actually visible? If yes, provide the histologic grade (1 or 2), "
                        f"tumor area (mm^2), estimated mass (g), and max diameter (mm)."
                    )
                else:
    ...
    def __getitem__(self, idx: int):
        ...
        final_input_image = self.image_transform(image_pil)
        
     
        mask_path = self.mask_paths[idx]
        if mask_path is not None:
            mask_file = Image.open(mask_path).convert("L")
            mask_tensor = self.mask_transform(mask_file)
            mask_tensor = (mask_tensor > 0).float()
        else:
            mask_tensor = torch.zeros((1, 336, 336), dtype=torch.float32)

        return final_input_image, mask_tensor, self.questions[idx], self.answers[idx]
...
if __name__ == "__main__":
    config = {
        ...
        "save_path": "/home/ealam/Desktop/llava-physics-text-bbox-vlm204_1",
    ...
    print("Step 1: Loading external segmentation model for BBox guidance...")
    if not os.path.exists(config['segmentation_model_path']):
        print(f"ERROR: Segmentation Weights not found at {config['segmentation_model_path']}")
        exit()
    
    ext_seg_model = get_external_segmentation_model()
    ext_seg_model.load_state_dict(torch.load(config['segmentation_model_path'], map_location=DEVICE))
    ext_seg_model.to(DEVICE).eval()
    ext_seg_transform = get_external_seg_transforms()
    print("External Segmentation Model Loaded.")
    ...
    print("\nStep 3: Setting up Multi-Task Model (VQA + Seg Head)...")
    ...
    print("\nStep 4: Preparing DataLoaders (Generating BBoxes via External Model)...")
   
    train_ds = VLM_Physics_Textual_BBox_Dataset(train_df, ext_seg_model, ext_seg_transform, DEVICE, is_train=True)
    val_ds = VLM_Physics_Textual_BBox_Dataset(val_df, ext_seg_model, ext_seg_transform, DEVICE, is_train=False)
    test_ds = VLM_Physics_Textual_BBox_Dataset(test_df, ext_seg_model, ext_seg_transform, DEVICE, is_train=False)
