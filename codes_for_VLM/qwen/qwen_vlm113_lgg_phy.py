...
class VLM_QASegDataset(Dataset):
    def __init__(self, metadata_df: pd.DataFrame, is_train: bool = True):
        ...
        print(f"Building VLM_QASegDataset from metadata: {len(metadata_df)} rows")
        ...
        print(f"Final dataset size: {len(self.image_paths)} samples verified.")
...
class QwenVLWithSegmentation(nn.Module):
    ...
    def _visual_to_grid(self, pixel_values: torch.Tensor, image_grid_thw: torch.Tensor, batch_size: int):
        ...
        S = N // batch_size
        H = W = int(math.sqrt(S))
        tokens = tokens.view(batch_size, S, C)
        feat_map = tokens.transpose(1, 2).contiguous().view(batch_size, C, H, W)
        return feat_map
    ...
    def forward(self, **batch):
        ...
        with autocast(enabled=True, dtype=torch.float16):
            ...
            seg_logits = F.interpolate(seg_logits_full, size=self.seg_out_size, mode="bilinear", align_corners=False).squeeze(1)
...
def discover_lora_targets(model, include_vision: bool = True) -> List[str]:
    ...
    for name, module in model.named_modules():
        if isinstance(module, (nn.Linear, nn.Conv2d)):
            ...
            if any(key == name_suffix for key in text_keys): target_suffixes.add(name_suffix)
            if include_vision and "visual" in name and any(key == name_suffix for key in vision_keys): target_suffixes.add(name_suffix)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "save_path": "/workspace/qwen/qwen_vlm113_multitask_physics",
        "lr": 1e-4, "batch_size": 2, "epochs": 25, "patience": 5, "seed": 42,
        "seg_weight": 3.0, "tumor_seg_weight": 1.0, "vision_lora": True, "grad_accum": 4
    }

    torch.manual_seed(config["seed"]); np.random.seed(config["seed"]); random.seed(config["seed"])

    print("Step 1: Loading Physics Metadata...")
    ...
    print("Step 2: Loading model and processor...")
    ...
    target_modules = discover_lora_targets(base_model, include_vision=config["vision_lora"])
    ...
    train_ds = VLM_QASegDataset(train_df)
    val_ds = VLM_QASegDataset(val_df)
    test_ds = VLM_QASegDataset(test_df)
    ...
    test_loader = DataLoader(test_ds, batch_size=config["batch_size"], collate_fn=vlm_seg_collate_fn)
    ...
    print("Step 4: Training Multi-task Qwen Physics...")
    for epoch in range(config["epochs"]):
        multitask_model.train()
        for step, (images, masks, questions, answers, has_tumors) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}")):
            batch_gpu = _to_device(build_training_batch_cpu_main(images, masks, questions, answers, has_tumors, processor), DEVICE)
            ...
            with autocast(enabled=True, dtype=torch.float16):
                ...
                weights[batch_gpu["has_tumor"]] = config["tumor_seg_weight"]
        ...
        if (val_acc + val_iou) > best_metric:
            best_metric = val_acc + val_iou; patience_count = 0
