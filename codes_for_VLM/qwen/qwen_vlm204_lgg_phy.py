...
def get_guidance_seg_model() -> nn.Module:
    """Loads the ResNet101 model used for pre-training delineation."""
...
def delineate_roi_on_image(pil_image: Image.Image, seg_model: nn.Module, seg_transform: A.Compose, device: str) -> Image.Image:
    """Draws a yellow contour on the image based on the guidance model's prediction."""
...
def compute_iou_batch(pred_logits: torch.Tensor, true_masks: torch.Tensor, threshold: float = 0.5) -> float:
    """Calculates Mean IoU for a batch - Restored for v204."""
    with torch.no_grad():
...
class VLM_QASegDataset(Dataset):
    def __init__(self, metadata_df: pd.DataFrame, guidance_model: nn.Module, guidance_transform: A.Compose, device: str):
        ...
        self.guidance_transform = guidance_transform
        self.device = device
        
        self.vlm_resize = transforms.Resize((336, 336))
        ...
        print(f"Building Dataset v204: {len(metadata_df)} rows")
    ...
    def __getitem__(self, idx: int):
        ...
        delineated_image = delineate_roi_on_image(raw_image, self.guidance_model, self.guidance_transform, self.device)
        ...
        return final_vlm_image, mask_tensor, self.questions[idx], self.answers[idx], self.has_tumors[idx]
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "save_path": "/workspace/qwen/qwen_vlm204_visual_guidance",
    ...
    print("Step 1: Loading Guidance Segmentation Model...")
    ...
    print("Step 2: Loading VLM and Processor...")
    ...
    print("Step 3: Preparing DataLoaders with Visual Guidance...")
    ...
    best_metric = 0.0
    print("Step 4: Starting Multi-task Training (v204)...")
