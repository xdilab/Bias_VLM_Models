...
class ClassificationEvaluator:
    ...
    def parse_grade(self, text):
        ...
        if g_match: return int(g_match.group(1))
        if "no tumor" in text or "grade: 0" in text: return 0
        return 0
        ...
        for _, row in tqdm(metadata_df.iterrows(), total=len(metadata_df), desc="Pre-calculating BBoxes"):
            ...
            if bbox:
                ...
                q = (f"Analyze this MRI slice. A potential lesion was identified at bounding box "
                     f"(x:{x}, y:{y}, w:{w}, h:{h}) in 256x256 space. Is a tumor visible? "
                     f"If yes, state the histologic grade (1 or 2).")
            else:
                q = ("Analyze this MRI slice. Is a tumor visible? If yes, state the histologic grade (1 or 2).")
...
class LlavaMultiTaskModel(nn.Module):
...
def run_evaluation(model, processor, data_loader, description="Evaluating"):
    ...
    with torch.no_grad():
        for imgs, masks, qs, ans in tqdm(data_loader, desc=description):
            ...
            with autocast(enabled=(DEVICE.type=="cuda"), dtype=dtype_to_use):
                ...
                gen_ids = model.llava.generate(input_ids=inf_batch["input_ids"], pixel_values=inf_batch["pixel_values"], attention_mask=inf_batch["attention_mask"], max_new_tokens=40)
            
            gen_ids = gen_ids[:, inf_batch["input_ids"].shape[1]:]
            clf_eval.update(ans, processor.batch_decode(gen_ids, skip_special_tokens=True))
    ...
    print(f"\n--- {description} --- \n  Acc: {m['Accuracy']*100:.2f}% | F1: {m['F1_Weighted']:.4f} | IoU: {avg_iou:.4f}")
...
if __name__ == "__main__":
    config = {
        ...
        "save_path": "/home/ealam/Desktop/vlm_textual_bbox_clf",
        "llm_lr": 2e-5, "seg_lr": 1e-4, "epochs": 20, "batch_size": 2, "grad_accum": 4, "seed": 42
    }
    ...
    model = LlavaMultiTaskModel(get_peft_model(base_llava, lora_cfg)).to(DEVICE)
    
    for param in model.vision_tower.parameters(): param.requires_grad = False

    llm_p = [p for n, p in model.named_parameters() if p.requires_grad and "llava" in n]
    seg_p = [p for n, p in model.named_parameters() if p.requires_grad and ("seg_head" in n or "proj" in n)]
    ...
    train_ds = VLM_DelineatedDataset(train_df, config["base_img_dir"], ext_seg, ext_transform, DEVICE)
    val_ds = VLM_DelineatedDataset(val_df, config["base_img_dir"], ext_seg, ext_transform, DEVICE)
    test_ds = VLM_DelineatedDataset(test_df, config["base_img_dir"], ext_seg, ext_transform, DEVICE)
