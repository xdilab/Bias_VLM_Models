...
# Device & Precision setup for L40S
...
def draw_yellow_lines(pil_image: Image.Image, seg_model: nn.Module, seg_transform: A.Compose, device: str) -> Image.Image:
    ...
    if contours:
        cv2.drawContours(open_cv_image, contours, -1, (0, 255, 255), 2) # Yellow Delineation
...
class VLM_PhysicsDelineatedDataset(Dataset):
    def __init__(self, metadata_df: pd.DataFrame, base_img_dir: str, ext_model: nn.Module, ext_transform: A.Compose, device: str):
        ...
        for _, row in tqdm(metadata_df.iterrows(), total=len(metadata_df), desc="Indexing Delineated Dataset"):
            ...
            if row['has_tumor']:
                a = (f"Yes, a tumor is visible in the delineated area. Grade: {int(row['grade'])}. "
...
def run_evaluation(model, processor, data_loader, description="Evaluating"):
    ...
    print(f"\n--- {description} Results ---")
    print(f"  Acc: {m['Grade_Acc']*100:.2f}% | Seg IoU: {avg_iou:.4f}")
...
if __name__ == "__main__":
    config = {
        ...
        "save_path": "/home/ealam/Desktop/vlm204_delineated_physics",
    ...
    print("Step 1: Initializing External Delineator...")
    ...
    print(f"\nStep 2: Loading VLM strictly on {DEVICE}...")
    ...
    train_ds = VLM_PhysicsDelineatedDataset(train_df, config["base_img_dir"], ext_seg_model, ext_seg_transform, DEVICE)
    val_ds = VLM_PhysicsDelineatedDataset(val_df, config["base_img_dir"], ext_seg_model, ext_seg_transform, DEVICE)
    test_ds = VLM_PhysicsDelineatedDataset(test_df, config["base_img_dir"], ext_seg_model, ext_seg_transform, DEVICE)
    ...
    print(f"\nStep 4: Training Delineated VLM 204...")
    optimizer.zero_grad()
    for epoch in range(config["epochs"]):
        ...
        for step, (imgs, masks, qs, ans) in enumerate(pbar):
            ...
            with autocast(enabled=(DEVICE.type == "cuda"), dtype=dtype_to_use):
                out = model(**batch)
                loss = (out["vqa_loss"] + 2.0 * seg_loss_fn(out["seg_logits"], batch["seg_masks_gt"].squeeze(1))) / config["grad_accum"]
        ...
        if score > best_score:
            ...
            print(f"Metrics Improved. Best Score: {best_score:.4f}. Model Saved.")
        else:
            patience_counter += 1
            if patience_counter >= config["patience"]: break

    print("\nStep 5: Final Benchmarking on Test Set...")
